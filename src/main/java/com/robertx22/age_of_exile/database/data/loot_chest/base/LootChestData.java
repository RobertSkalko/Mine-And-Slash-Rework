...
public class LootChestData implements ICommonDataItem<GearRarity> {
    ...
    public void BuildTooltip(TooltipContext ctx) {
        ...
        tip.addAll(new ExileTooltips()
                ...
                .accept(new AdditionalBlock(ImmutableList.of(Chats.OPEN_LOOT_CHEST.locName().withStyle(ChatFormatting.GRAY))))
                .accept(new AdditionalBlock(() -> {
                    //weird
                    if (getKeyItem() != null) {
                        return ImmutableList.of(Itemtips.NEED_KEY.locName(getKeyItem().getDefaultInstance().getHoverName()));
                    } else {
                        return EMPTY_LIST;
                    }
                }).showWhen(this::isLocked))
                .accept(new LeveledItemLevelBlock(lvl))
                .release());
