...
public class BackpackMenu extends AbstractContainerMenu {
    ...
    public BackpackMenu(int pContainerId, Inventory inv) {
        this(pContainerId, inv, new SimpleContainer(6 * 9), 6);
    }

    public BackpackMenu(int pContainerId, Container playerINV, Container backpackINV, int containerRows) {
        ...
        try {
            ...
            for (int j = 0; j < containerRows; ++j) {
                for (int k = 0; k < 9; ++k) {
                    this.addSlot(new Slot(backpackINV, k + j * 9, 8 + k * 18, 18 + j * 18));
                }
            }
            for (int l = 0; l < 3; ++l) {
